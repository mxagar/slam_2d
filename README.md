...
This repository contains a toy implementation of Simultaneous Localization and Mapping (SLAM) in 2D which uses the [Graph SLAM Algorithm](http://robots.stanford.edu/papers/thrun.graphslam.pdf). A map of the environment is created only from sensor and motion data obtained from a simulated robot moving over time and space. The location of the robot is tracked, as well as the landmarks in the scene.

The starter code was taken from a project in the [Udacity Computer Vision Nanodegree](https://www.udacity.com/course/computer-vision-nanodegree--nd891), available in its original (incomplete) form in the repository [udacity/P3_Implement_SLAM](https://github.com/udacity/P3_Implement_SLAM).
...
```
.
├── 1. Robot Moving and Sensing.ipynb           # Notebook 1: Robot class explanation
├── 2. Omega and Xi, Constraints.ipynb          # Notebook 2: Graph SLAM explanation
├── 3. Landmark Detection and Tracking.ipynb    # Notebook 3: SLAM implementation
...
- `2. Omega and Xi, Constraints.ipynb`: the key aspects of the Graph SLAM method are explained for the 1D case.
- `3. Landmark Detection and Tracking.ipynb`: **that's where SLAM is implemented in 2D**.

On the other hand, the implementation scripts and their contents are the following:

- [`robot_class.py`](robot_class.py): `robot` class which contains the location of the robot in a 2D world; additionally, `move()` and `sense()` functions that enable interacting with the world are implemented. The landmarks measured in the world are created here.
...
- `measure()`, which yields the distance from the robot position to reachable landmarks, accounting for measurement error,
- and `move()`, which updates the robot position given a relative motion vector, accounting for movement error.
...
Each motion or measurement constraint is a Gaussian; for instance, in a 1D world in which we go from `x0` to `x1` with a step size of `dx` and a motion error of `sigma`, the constraint equation is given by:
...
- and the `xi` vector, which contains the distances between two consecutive locations or a location and a landmark.

The following figure shows how `Omega` (in blue) and `xi` (in pink) are assembled. Let's say we have a 1D world with 2 landmarks `L0, L1` and three robot positions/locations `x0, x1, x2`. Then, if we go from `x0` to `x1` with a `dx = 5`, assuming motion error `sigma = 1`, the matrices are filled as follows:
...
If we measure the distance from location/position `x1` to landmark `L1`, the reduced constraint equation has the following form:

$$ \frac{L_1}{\sigma} - \frac{x_1}{\sigma} = \frac{dL}{\sigma}.$$

This measurement constraint is equivalent to the former motion constraint, and it is entered to the constraint matrices `Omega` and `xi` in the same fashion; only now the uncertainty `sigma` refers to the measurement error instead of the motion error.

**Now the best part**: It turns out that the vector `mu` which contains the optimal solution that describes in world coordinates (1) the robot path, (2) its final location and (3) the location of the landmarks is the following:

$$ \mu = (x_0, x_1, \dots, x_n, L_0, L_1, \dots, L_m)^{T} = \Omega^{-1}\xi.$$

As simple as that! We aggregate in the matrices the different constraint coefficients and distances that occur along the time, we invert `omega` and a simple product gives the location and the map :sunglasses:.
...
- [My notes and code](https://github.com/mxagar/computer_vision_udacity) on the [Udacity Computer Vision Nanodegree](https://www.udacity.com/course/computer-vision-nanodegree--nd891). A more detailed explanation of the Graph SLAM algorithm is given in the folder [`04_Object_Tracking_Localization`](https://github.com/mxagar/computer_vision_udacity/blob/main/04_Object_Tracking_Localization/CVND_Object_Tracking_Localization.md).
